  ...
  */
  ...
  export class Svm{
    ...
    train(data: [[number, number]], labels: number[], options) {
      ...
      // parameters
      this.options = {};
      var C = this.options.C || 1.0; // C value. Decrease for more regularization
      var tol = this.options.tol || 1e-4; // numerical tolerance. Don't touch unless you're pro
      ...
      // initializations
      this.kernel = kernel;
      this.N = this.data.length; var N = this.N;
      this.D = this.data[0].length; var D = this.D;
      this.alpha = zeros(N);
      this.b = 0.0;
      ...
      if (this.options.memoize) {
        ...
        for (var i=0;i<N;i++) {
          ...
          for (var j=0;j<N;j++) {
            this.kernelResults[i][j] = kernel(this.data[i],this.data[j]);
          }
        }
      }
      ...
      while(passes < numpasses && iter < maxiter) {
        ...
        for(var i=0;i<N;i++) {
          ...
          if( (this.labels[i]*Ei < -tol && this.alpha[i] < C)
           || (this.labels[i]*Ei > tol && this.alpha[i] > 0) ){

            // alpha_i needs updating! Pick a j to update it with
            var j = i;
            while(j === i) j= randi(0, this.N);
            var Ej= this.marginOne(this.data[j]) - this.labels[j];
            ...
            var b1 = this.b - Ei - this.labels[i]*(newai-ai)*this.kernelResult(i,i)
                     - this.labels[j]*(newaj-aj)*this.kernelResult(i,j);
            var b2 = this.b - Ej - this.labels[i]*(newai-ai)*this.kernelResult(i,j)
                     - this.labels[j]*(newaj-aj)*this.kernelResult(j,j);
            this.b = 0.5*(b1+b2);
